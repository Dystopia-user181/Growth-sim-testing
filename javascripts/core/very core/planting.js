...
setInterval(function () {
	...
	player.money = player.money.add(prevHoney.mul(player.automator.sellHoney).mul(Decimal.pow(1.2, player.marketing).mul(plantSell/200)));
...
function sell() {
	if (player.plantPicked.lt(1)) return;
...
function sell10() {
	if (player.plantPicked.lt(10)) return;
	player.plantPicked = player.plantPicked.sub(10);
